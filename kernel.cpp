...
void os::Kernel::updateStatus(std::string command, os::Process* process){
    ...
    if (command == "boot" || command == "system call"){
        ...
        this->printKernelStatus();
        this->cycle++;      
        // during the sleep cycle, "idle" if the ready que is empty, else "schedule". 
        // sleep cycle starts to decrease since the systemcall
        // cycle+2] schedule
        while(SLEEP_CNT--){
        ...
        if (this->kernel_status.process_running!= NULL) return;
...
void os::Kernel::newProcess(std::string pName, int ppid){
    os::Process* newProcess = new Process(pName, 0, ppid+1, ppid, '0');
    // cycle+0]
...
void os::Kernel::run(){
    std::string program="init";
    int ppid = 0;
    bool beginNewProcess=true;

    while(true){
        if(beginNewProcess){
            beginNewProcess=false;
            this->newProcess(program, ppid);
        }
        else{
            if (this->kernel_status.process_running == NULL){
                break;
